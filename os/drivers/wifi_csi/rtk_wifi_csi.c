...
static int rtk_wifi_csi_open(FAR struct wifi_csi_lowerhalf_s *dev);
static int rtk_wifi_csi_close(FAR struct wifi_csi_lowerhalf_s *dev);
...
static int rtk_wifi_csi_open(FAR struct wifi_csi_lowerhalf_s *dev)
{
	csivdbg("lower driver open\n");
    return 0;
}
static int rtk_wifi_csi_close(FAR struct wifi_csi_lowerhalf_s *dev)
{
    csivdbg("lower driver close\n");
    return 0;
}
...
static int rtk_wifi_csi_ioctl(FAR struct wifi_csi_lowerhalf_s *dev, int cmd, unsigned long arg)
{
	...
	case CSIIOC_REGISTER_CALLBACK: {
		...
		rtk_wifi_csi_takesem(&priv->devsem);
		wifi_reg_event_handler(21, data_ready_listener, dev);
		rtk_wifi_csi_givesem(&priv->devsem);
	...
	case CSIIOC_UNREGISTER_CALLBACK: {
		...
		rtk_wifi_csi_takesem(&priv->devsem);
		wifi_unreg_event_handler(21, data_ready_listener);
		rtk_wifi_csi_givesem(&priv->devsem);
